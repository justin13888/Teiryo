...
- **Assumed response schema** (checked against a live Max response, kept as the parser's `MAX_WITH_FABLE` test fixture; fields beyond those named below are still ignored, and the parser returns `ParseError::SchemaDrift` naming what's missing rather than guessing): top-level buckets `five_hour`, `seven_day`, and Max-only `seven_day_opus` / `seven_day_sonnet`, each `{ "utilization": <percent used 0–100>, "resets_at": <ISO 8601 | null> }`; plus a `limits` array whose rows carry `kind`, `percent`, `resets_at` and `scope`. A `weekly_scoped` row with `scope.model.display_name` (e.g. `"Fable"`) is a per-model weekly cap that has no top-level bucket — the only place the Fable weekly limit appears on Max. Rows of other kinds restate the fixed buckets and are skipped, as is a row that names a model a fixed bucket already covers; a row in an unexpected shape (or a `limits` field that is `null` or not an array) is dropped on its own with a warning rather than failing the poll, and so is a recognised row missing `percent`, unlike a bucket missing `utilization`, which is `SchemaDrift`. Unknown fields and buckets are ignored. Mapped windows: `session_5h` (Rolling 5 h), `weekly` (Rolling 7 d), `weekly_opus`/`weekly_sonnet` (`WindowScope::Model(..)`), and `weekly_<model>` from each `weekly_scoped` row — `weekly_fable` with `WindowScope::Model("fable")`, label `Weekly — Fable`, where the slug is the display name lower-cased with non-alphanumeric runs as `_`; always `QuotaUnit::Percent`, `limit: Some(100.0)`. Fixed buckets come first, then `limits[]` windows in server order.
